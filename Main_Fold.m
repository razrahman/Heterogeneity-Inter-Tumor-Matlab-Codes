...
Cancer1_AUC=num(cancer_cell_number{8},12)/8;
Cancer1_cell=txt(cancer_cell_number{8}+1,1);
Cancer2_AUC=num(cancer_cell_number{15},12)/8;
Cancer2_cell=txt(cancer_cell_number{15}+1,1);
%%
Ind1=[];
...
% Sample_size_Cancer1=size(Cancer1_AUC,1); Sample_size_Cancer2=size(Cancer2_AUC,1);
...
Y_hat_combined=[]; Y_hat_combined_LDA=[]; Y_hat_combined_DT=[]; Y_hat_combined_KNN=[];
combined_AUC_test=[];
for FF=1:F
    ...
    combined_gene_train1=combined_gene_train(:,ranked_combined(1:40));
    combined_gene_test1=combined_gene_test(:,ranked_combined(1:40));
    ...
    MdlLinear = fitcdiscr(combined_gene_train1,combined_AUC_train(:,2));
    ...
    Decision_Tree = fitctree(combined_gene_train1,combined_AUC_train(:,2),'MinLeafSize',min_leaf);
    Decision_Tree_classify= predict(Decision_Tree,combined_gene_test1);
    Y_hat_combined_DT = [Y_hat_combined_DT; forest_predict(combined_gene_test,model_combined,Decision_Tree_classify)];
    %% Fit K-nearnest neighbor classifier
    MdlKNN = fitcknn(combined_gene_train1,combined_AUC_train(:,2));
    KNN_classify=predict(MdlKNN,combined_gene_test1);
    Y_hat_combined_KNN = [Y_hat_combined_KNN; forest_predict(combined_gene_test,model_combined,KNN_classify)];
end
...
Result_KNN=[mean((combined_AUC_test(:,1)-Y_hat_combined_KNN(:,1)).^2) mean(abs(combined_AUC_test(:,1)-Y_hat_combined_KNN(:,1)))...
        mean((combined_AUC_test(:,1)-Y_hat_combined_KNN(:,2)).^2) mean(abs(combined_AUC_test(:,1)-Y_hat_combined_KNN(:,2)))]
    
Classify=[combined_AUC_test(:,2) Y_hat_combined(:,3) Y_hat_combined_LDA(:,3) Y_hat_combined_DT(:,3) Y_hat_combined_KNN(:,3)];
for i=1:4
