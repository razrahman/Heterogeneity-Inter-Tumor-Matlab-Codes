...
Y_hat_combined=[]; Y_hat_combined_LDA=[];  Y_hat_combined_LDA_B=[];%Y_hat_combined_DT=[]; Y_hat_combined_KNN=[];
combined_AUC_test=[];
for FF=1:F
    ...
    combined_gene_train1=combined_gene_train(:,ranked_combined(1:500));
    combined_gene_test1=combined_gene_test(:,ranked_combined(1:500));
    ...
    Y_hat_combined_LDA_B = [Y_hat_combined_LDA_B; check_tree(model_combined,combined_gene_train,combined_AUC_train,LDA_classify,combined_gene_test)];
end
...
Result_LDA_B=[mean((combined_AUC_test(:,1)-Y_hat_combined_LDA_B(:,1)).^2) mean(abs(combined_AUC_test(:,1)-Y_hat_combined_LDA_B(:,1)))...
        mean((combined_AUC_test(:,1)-Y_hat_combined_LDA_B(:,2)).^2) mean(abs(combined_AUC_test(:,1)-Y_hat_combined_LDA_B(:,2)))]
    
Classify=[combined_AUC_test(:,2) Y_hat_combined(:,3) Y_hat_combined_LDA(:,3)];
for i=1:2
